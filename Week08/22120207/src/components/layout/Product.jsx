...
const Product = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data, count } = await GetProducts(start, end);
    ...
    <div className="w-full bg-[#f4f8ff] p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Đang hiển thị trang {currentPage}/{totalPages || 1} - mỗi trang{" "}
          {itemsPerPage} sản phẩm.
        </p>
      </div>
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 select-none group hover:cursor-pointer">
          {/* Previous Button */}
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            disabled={currentPage === 1}
            className="p-2 rounded-full hover:bg-slate-200 text-slate-600 transition disabled:opacity-50 disabled:hover:bg-transparent disabled:pointer-events-none"
          >
